...
路由均以 map* 开头, 例如 endpoints.mapGet() .mapPost() .mapControllers()
...
GetEndPoint()需要在 UseRouting() 匹配路由之后 执行才可以获取到之后会执行的端点, 否则为null, 为匹配到路由也为null

**可以实现例如: 仅在达到特定端点时记录某些日志**, 但是端点对象 Microsoft.AspNetCore.Http.Endpoint 好像获取不到什么太有用的信息, 例如请求路径(只有一个 DisplayName 但还包含 HTTP xxx, 不是纯粹的路由路径), 如果可以获取到请求路径就可以根据路径前缀来记录特定日志 或 请求参数了
...
app.Use(async (context, next) =>
{
    ...
    if (endpoint != null)// != null 说明路由匹配到了
    {
        // log.info();
    }
    await next(context);// 继续下一个中间件
